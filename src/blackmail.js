class Blackmail {
	constructor(str, elem, color, shadow, shadow_color, bg_color, animated) {
		this.str = str;
		this.elem = elem;
		this.color = color;
		this.shadow = shadow;
		...
		this.tags = ["p class='blk_p'", "strong class='blk_strong'", "i class='blk_i'",
	...
	animate() {
		...
		return (function() {
		  ...
		  $("<div />").appendTo("body");
		  _animateID = requestAnimationFrame(repeatOften);
		}
	...
	rndRGBA() {
		...
		let _result = "rgba(" + _r + "&#44;" + _g + "&#44;" + _b + "&#44;" + _range + ")";
		return _result;
	};
	generate() {
		let _composition = "";
		for (let i = 0; i <= this.len - 1; i++) {
			...
			var _char = "<" + _tag + " style='float:left;line-height:" + _lh.toFixed(2) +
				";letter-spacing:.24em;";
